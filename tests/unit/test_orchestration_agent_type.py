...
class TestParseEnvIntErrorMessageTemplates:
    ...
    def test_log_message_contains_negative_variable_value_and_default(
        ...
        """Info log should contain the variable name with negative override value and default.

        Verifies the message includes TEST_TMPL_VAR=-5 (negative override)
        and default: -1 (negative default value).
        """
        with mock.patch.dict("os.environ", {"TEST_TMPL_VAR": "-5"}):
            with caplog.at_level(logging.INFO, logger="sentinel.orchestration"):
                _parse_env_int("TEST_TMPL_VAR", -1, min_value=-10)
